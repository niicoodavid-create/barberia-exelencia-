...
function turnoVencido(v){return v<=fechaHoraArgentina()}
...
async function marcarTurnosVencidos(){
 try{
  await pool.query(`UPDATE turnos SET estado='vencido' WHERE estado='confirmado' AND fecha_hora<=$1`,[fechaHoraArgentina()]);
 }catch(e){
...
async function iniciarBD(){
 ...
 try{await pool.query(`DROP INDEX IF EXISTS idx_turnos_uno_por_cliente_confirmado`)}catch(e){console.warn('Indice anterior de cliente no eliminado:',e.message)}
 try{await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_turnos_fecha_hora_confirmado ON turnos(fecha_hora) WHERE estado='confirmado'`)}catch(e){console.warn('Indice de horario no creado:',e.message)}
 const barber=await pool.query('SELECT id FROM clientes WHERE lower(email)=lower($1) LIMIT 1',[ADMIN_EMAIL]);
 if(barber.rows.length)await pool.query(`UPDATE clientes SET rol='barbero' WHERE id=$1`,[barber.rows[0].id]);
 await marcarTurnosVencidos();
 console.log('Base de datos de Excelencia lista.');
}
app.get('/',(req,res)=>res.sendFile(path.join(__dirname,'index.html')));
app.get('/api/configuracion',(req,res)=>res.json({direccion:'Paraguay 176',horarios:HORAS_LABORALES,ventana_dias:14,correo_barbero:ADMIN_EMAIL}));
...
app.post('/api/logout-admin',(req,res)=>{
 res.setHeader('Set-Cookie',['admin_session=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0','cliente_session=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0']);
 res.json({ok:true});
});
app.get('/auth/google',(req,res)=>{
 if(!CLIENT_ID||!CLIENT_SECRET)return res.status(500).send('Google OAuth no está configurado.');
 const p=new URLSearchParams({client_id:CLIENT_ID,redirect_uri:GOOGLE_REDIRECT_URI,response_type:'code',scope:'openid email profile',access_type:'online',prompt:'select_account'});
...
app.get('/auth/google/callback',async(req,res)=>{
 ...
 if(!CLIENT_ID||!CLIENT_SECRET)return res.status(500).send('Google OAuth no está configurado.');
 try{
  const tr=await fetch('https://oauth2.googleapis.com/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({code,client_id:CLIENT_ID,client_secret:CLIENT_SECRET,redirect_uri:GOOGLE_REDIRECT_URI,grant_type:'authorization_code'})});
  ...
  if(q.rows.length){
   ...
   q=await pool.query(`INSERT INTO clientes(nombre,email,rol) VALUES($1,$2,$3) RETURNING *`,[nombre,email,email===ADMIN_EMAIL?'barbero':'cliente']);
   usuario=q.rows[0];
  }
  await marcarTurnosVencidos();
  const cookies=[cookieCliente(usuario.id)];
  if(email===ADMIN_EMAIL)cookies.push(`admin_session=${encodeURIComponent(firmarAdmin())}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=43200`);
  else cookies.push('admin_session=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0');
...
app.put('/api/admin/servicios/:id',requiereAdmin,async(req,res)=>{
 const id=Number(req.params.id),nombre=String(req.body.nombre||'').trim(),precio=Number(req.body.precio);
...
app.delete('/api/admin/servicios/:id',requiereAdmin,async(req,res)=>{
 ...
 try{
  ...
  if(usados.rows.length)return res.status(409).json({error:'No se puede borrar un servicio que tiene historial de turnos.'});
...
app.get('/api/turnos',async(req,res)=>{
 try{
  ...
  const r=await pool.query(`
   SELECT
   t.id,t.clientes_id,t.servicios_id,t.fecha_hora,t.estado,
   c.nombre cliente,c.email cliente_email,
   s.nombre servicio,s.precio precio
   FROM turnos t
...
async function crearTurno(clienteId,servicioId,fechaHora){
 await marcarTurnosVencidos();
 if(!Number.isInteger(clienteId)||!Number.isInteger(servicioId)||!esFechaHoraValida(fechaHora))return{status:400,error:'Los datos del turno no son válidos.'};
 const fecha=fechaHora.slice(0,10);
 if(fecha<fechaArgentina()||fecha>fechaMasDias(14)||turnoVencido(fechaHora))return{status:409,error:'Ese horario ya no está disponible.'};
 const client=await pool.connect();
 try{
  await client.query('BEGIN');
  await client.query('SELECT pg_advisory_xact_lock(hashtext($1))',[fechaHora]);
  await client.query('SELECT pg_advisory_xact_lock(hashtext($1))',[`cliente:${clienteId}`]);
  const b=await client.query('SELECT 1 FROM dias_bloqueados WHERE fecha=$1 LIMIT 1',[fecha]);
  if(b.rows.length){
   await client.query('ROLLBACK');
   return{status:409,error:'Ese día está cerrado.'};
  }
  const c=await client.query('SELECT id FROM clientes WHERE id=$1 LIMIT 1',[clienteId]);
  if(!c.rows.length){
   await client.query('ROLLBACK');
   return{status:400,error:'El cliente no existe.'};
  }
  const s=await client.query('SELECT id FROM servicios WHERE id=$1 LIMIT 1',[servicioId]);
  if(!s.rows.length){
   await client.query('ROLLBACK');
   return{status:400,error:'El servicio no existe.'};
  }
  await client.query(`UPDATE turnos SET estado='vencido' WHERE clientes_id=$1 AND estado='confirmado' AND fecha_hora<=$2`,[clienteId,fechaHoraArgentina()]);
  const own=await client.query(`SELECT fecha_hora FROM turnos WHERE clientes_id=$1 AND estado='confirmado' AND fecha_hora>$2 LIMIT 1`,[clienteId,fechaHoraArgentina()]);
  if(own.rows.length){
   await client.query('ROLLBACK');
   return{status:409,error:'Ya tienes un turno activo. No puedes reservar otro.'};
  }
  const busy=await client.query(`SELECT id FROM turnos WHERE fecha_hora=$1 AND estado='confirmado' LIMIT 1`,[fechaHora]);
  if(busy.rows.length){
   await client.query('ROLLBACK');
   return{status:409,error:'Ese horario ya está ocupado.'};
  }
  const n=await client.query(`INSERT INTO turnos(clientes_id,servicios_id,fecha_hora,estado) VALUES($1,$2,$3,'confirmado') RETURNING *`,[clienteId,servicioId,fechaHora]);
  await client.query('COMMIT');
  return{status:201,row:n.rows[0]};
 }catch(e){
  try{await client.query('ROLLBACK')}catch{}
  if(e.code==='23505')return{status:409,error:'Ese horario o ese turno ya no está disponible.'};
  console.error(e);
  return{status:500,error:'No se pudo reservar el turno.'};
 }finally{
  client.release();
 }
}
app.post('/api/turnos',requiereCliente,async(req,res)=>{
 const r=await crearTurno(req.clienteId,Number(req.body.servicios_id),typeof req.body.fecha_hora==='string'?req.body.fecha_hora.trim():'');
 res.status(r.status).json(r.status===201?r.row:{error:r.error});
});
app.post('/api/admin/turnos',requiereAdmin,async(req,res)=>{
 const r=await crearTurno(Number(req.body.cliente_id),Number(req.body.servicios_id),typeof req.body.fecha_hora==='string'?req.body.fecha_hora.trim():'');
 res.status(r.status).json(r.status===201?r.row:{error:r.error});
});
app.put('/api/admin/turnos/:id',requiereAdmin,async(req,res)=>{
 const id=Number(req.params.id),clienteId=Number(req.body.cliente_id),servicioId=Number(req.body.servicios_id),fechaHora=typeof req.body.fecha_hora==='string'?req.body.fecha_hora.trim():'';
 if(!Number.isInteger(id)||!Number.isInteger(clienteId)||!Number.isInteger(servicioId)||!esFechaHoraValida(fechaHora))return res.status(400).json({error:'Datos inválidos.'});
 await marcarTurnosVencidos();
 const client=await pool.connect();
 try{
  await client.query('BEGIN');
  ...
  await client.query('SELECT pg_advisory_xact_lock(hashtext($1))',[`cliente:${clienteId}`]);
  const fecha=fechaHora.slice(0,10);
  if(fecha<fechaArgentina()||fecha>fechaMasDias(14)||turnoVencido(fechaHora)){
   await client.query('ROLLBACK');
  ...
  const existe=await client.query(`SELECT id,clientes_id FROM turnos WHERE id=$1 LIMIT 1`,[id]);
  if(!existe.rows.length){
   await client.query('ROLLBACK');
   return res.status(404).json({error:'Turno no encontrado.'});
  }
  const busy=await client.query(`SELECT id FROM turnos WHERE fecha_hora=$1 AND estado='confirmado' AND id<>$2 LIMIT 1`,[fechaHora,id]);
  if(busy.rows.length){
   await client.query('ROLLBACK');
   return res.status(409).json({error:'Ese horario ya está ocupado.'});
  }
  const own=await client.query(`SELECT id FROM turnos WHERE clientes_id=$1 AND estado='confirmado' AND fecha_hora>$2 AND id<>$3 LIMIT 1`,[clienteId,fechaHoraArgentina(),id]);
  if(own.rows.length){
   await client.query('ROLLBACK');
  ...
  const u=await client.query(`UPDATE turnos SET clientes_id=$1,servicios_id=$2,fecha_hora=$3,estado='confirmado' WHERE id=$4 RETURNING *`,[clienteId,servicioId,fechaHora,id]);
  await client.query('COMMIT');
  res.json(u.rows[0]);
 }catch(e){
  try{await client.query('ROLLBACK')}catch{}
  if(e.code==='23505')return res.status(409).json({error:'No fue posible guardar: horario ocupado.'});
  console.error(e);
...
app.delete('/api/admin/turnos/:id',requiereAdmin,async(req,res)=>{
 ...
 if(!Number.isInteger(id))return res.status(400).json({error:'Id inválido.'});
 try{
  const r=await pool.query(`UPDATE turnos SET estado='cancelado' WHERE id=$1 AND estado='confirmado' RETURNING *`,[id]);
  if(!r.rows.length)return res.status(404).json({error:'Turno no encontrado o ya cancelado.'});
  res.json(r.rows[0]);
 }catch(e){
  console.error(e);
...
app.get('/api/clientes',requiereAdmin,async(req,res)=>{
 try{
  await marcarTurnosVencidos();
  const r=await pool.query(`
   SELECT c.id,c.nombre,c.email,c.rol,
   COUNT(t.id)::int turnos_total,
   COUNT(t.id) FILTER(WHERE t.estado='confirmado' AND t.fecha_hora>$1)::int turnos_activos,
...
app.put('/api/admin/clientes/:id',requiereAdmin,async(req,res)=>{
 const id=Number(req.params.id),nombre=nombreCapitalizado(req.body.nombre),email=String(req.body.email||'').trim().toLowerCase();
 if(!Number.isInteger(id)||!nombre||!email)return res.status(400).json({error:'Nombre y email son obligatorios.'});
 try{
  const actual=await pool.query('SELECT id,email,rol FROM clientes WHERE id=$1 LIMIT 1',[id]);
  if(!actual.rows.length)return res.status(404).json({error:'Cliente no encontrado.'});
  if(actual.rows[0].email.toLowerCase()===ADMIN_EMAIL)return res.status(403).json({error:'No se puede modificar la cuenta del administrador.'});
  if(email===ADMIN_EMAIL)return res.status(400).json({error:'Ese correo pertenece al administrador.'});
  const dup=await pool.query('SELECT id FROM clientes WHERE lower(email)=lower($1) AND id<>$2 LIMIT 1',[email,id]);
  if(dup.rows.length)return res.status(409).json({error:'Ese email ya está registrado.'});
  const r=await pool.query(`UPDATE clientes SET nombre=$1,email=$2 WHERE id=$3 RETURNING id,nombre,email,rol`,[nombre,email,id]);
  res.json(r.rows[0]);
 }catch(e){
  console.error(e);
...
app.delete('/api/admin/clientes/:id',requiereAdmin,async(req,res)=>{
 ...
 if(!Number.isInteger(id))return res.status(400).json({error:'Id inválido.'});
 try{
  const u=await pool.query('SELECT id,email FROM clientes WHERE id=$1 LIMIT 1',[id]);
  if(!u.rows.length)return res.status(404).json({error:'Cliente no encontrado.'});
  if(u.rows[0].email.toLowerCase()===ADMIN_EMAIL)return res.status(403).json({error:'No se puede eliminar al administrador.'});
  const t=await pool.query('SELECT 1 FROM turnos WHERE clientes_id=$1 LIMIT 1',[id]);
  if(t.rows.length)return res.status(409).json({error:'No se puede eliminar un cliente con historial de turnos.'});
...
app.get('/api/dias-bloqueados',async(req,res)=>{
 try{
  const r=await pool.query('SELECT fecha FROM dias_bloqueados ORDER BY fecha');
  res.json(r.rows.map(x=>x.fecha));
 }catch(e){
  console.error(e);
  res.status(500).json({error:'No se pudieron obtener los días cerrados.'});
 }
});
app.post('/api/admin/dias-bloqueados',requiereAdmin,async(req,res)=>{
 const fecha=String(req.body.fecha||'').trim();
 if(!esFechaValida(fecha)||fecha<fechaArgentina()||fecha>fechaMasDias(14))return res.status(400).json({error:'Solo puedes cerrar días dentro de los próximos 14 días.'});
 try{
  const r=await pool.query(`INSERT INTO dias_bloqueados(fecha) VALUES($1) ON CONFLICT(fecha) DO NOTHING RETURNING fecha`,[fecha]);
  res.status(201).json({fecha,creada:r.rows.length>0});
 }catch(e){
  console.error(e);
...
app.delete('/api/admin/dias-bloqueados/:fecha',requiereAdmin,async(req,res)=>{
 ...
 try{
  const r=await pool.query('DELETE FROM dias_bloqueados WHERE fecha=$1 RETURNING fecha',[fecha]);
  if(!r.rows.length)return res.status(404).json({error:'Ese día ya estaba disponible.'});
  res.json({ok:true,fecha});
 }catch(e){
  console.error(e);
  res.status(500).json({error:'No se pudo abrir el día.'});
 }
});
app.get('/api/admin/estado',requiereAdmin,async(req,res)=>{
 try{
  ...
  const[c,t,s,d]=await Promise.all([
   ...
   pool.query(`SELECT COUNT(*)::int total FROM turnos WHERE estado='confirmado' AND fecha_hora>$1`,[fechaHoraArgentina()]),
  ...
  res.json({clientes:c.rows[0].total,turnos_activos:t.rows[0].total,servicios:s.rows[0].total,dias_cerrados:d.rows[0].total});
 }catch(e){
  console.error(e);
  res.status(500).json({error:'No se pudo obtener el estado.'});
 }
});
iniciarBD().then(()=>app.listen(port,()=>console.log(`Servidor de Excelencia escuchando en ${port}`))).catch(e=>{console.error('Error inicializando la base:',e);process.exit(1)});