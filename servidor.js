...
async function iniciarBD(){
 ...
 if(barber.rows.length){
  await pool.query(
   `UPDATE clientes SET rol='barbero' WHERE id=$1`,
   [barber.rows[0].id]
  );
 }
...
app.post('/api/login',async(req,res)=>{
 ...
 try{
  const ex=await pool.query(
   'SELECT * FROM clientes WHERE lower(email)=lower($1) LIMIT 1',
   [email]
  );
  ...
  if(ex.rows.length){
   ...
   await pool.query(
    'UPDATE clientes SET nombre=$1 WHERE id=$2',
    [nombre,u.id]
   );
  ...
  const n=await pool.query(`
   INSERT INTO clientes(nombre,email,rol)
   VALUES($1,$2,'cliente')
   RETURNING *
  `,[nombre,email]);
 ...
 try{
  ...
  if(q.rows.length){
   ...
   `,[
    nombre,
    email,
    email===ADMIN_EMAIL?'barbero':'cliente'
   ]);
  ...
  if(email===ADMIN_EMAIL){
   cookies.push(
    `admin_session=${encodeURIComponent(firmarAdmin())}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=43200`
   );
  }else{
   cookies.push(
    'admin_session=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0'
  ...
  res.send(`
   ...
   localStorage.setItem(
    'usuarioActivo',
    ${JSON.stringify(JSON.stringify(usuario))}
   );
...
app.delete('/api/admin/servicios/:id',requiereAdmin,async(req,res)=>{
 ...
 try{
  const usados=await pool.query(
   'SELECT 1 FROM turnos WHERE servicios_id=$1 LIMIT 1',
   [id]
  );

  if(usados.rows.length){
   return res.status(409).json({
    error:'No se puede borrar un servicio que tiene historial de turnos.'
   });
  }
...
app.post('/api/turnos',requiereCliente,async(req,res)=>{
 const r=await crearTurno(
  ...
  typeof req.body.fecha_hora==='string'
   ?req.body.fecha_hora.trim()
   :''
 );
...
app.put('/api/admin/turnos/:id',requiereAdmin,async(req,res)=>{
 ...
 const fechaHora=typeof req.body.fecha_hora==='string'
  ?req.body.fecha_hora.trim()
  :'';
 ...
 try{
  ...
  res.status(500).json({
   error:'No se pudo editar el turno.'
  });
 ...
 try{
  ...
  if(actual.rows[0].email.toLowerCase()===ADMIN_EMAIL){
   return res.status(403).json({
    error:'No se puede modificar la cuenta del administrador.'
   });
  }
...
app.post('/api/admin/dias-bloqueados',requiereAdmin,async(req,res)=>{
 ...
 if(
  ...
  return res.status(400).json({
   error:'Solo puedes cerrar días dentro de los próximos 14 días.'
  });
 }
 ...
 try{
  ...
  const[c,t,s,d]=await Promise.all([
   ...
   pool.query(`
    ...
    WHERE fecha>=$1
    AND fecha<=$2
   `,[fechaArgentina(),fechaMasDias(14)])
  ]);
